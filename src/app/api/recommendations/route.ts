...
async function buildRecommendationsPayload(
  user: { riskProfile: keyof typeof RISK_SCORE_TARGETS },
  portfolioId: string
) {
  ...
  const profileKey = (data.riskProfile ?? user.riskProfile) as keyof typeof RISK_PROFILE_LABELS;

  return {
    ...data,
    riskProfileLabel: RISK_PROFILE_LABELS[profileKey] ?? RISK_PROFILE_LABELS.BALANCED,
    riskBand: RISK_SCORE_TARGETS[user.riskProfile] ?? RISK_SCORE_TARGETS.BALANCED,
...
export async function GET() {
  try {
    ...
    return jsonOk(await buildRecommendationsPayload(user, portfolioId));
  } catch (error) {
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (refresh) {
      ...
      const payload = await buildRecommendationsPayload(user, portfolioId);
      return jsonOk({ refreshed: true, run: result, ...payload });
    }
